...
const ThemeFont = () => {
  ...
  return (
    <RadioGroup row value={fontFamily} onChange={handleFontChange}>
          ...
          <Grid item key={index}>
            <FormControlLabel
              ...
              label={
                <MainCard
                  content={false}
                  sx={{ bgcolor: fontFamily === item.value ? '#e6f7ff' : '#f5f5f5', p: 1 }}
                  border={false}
                  {...(fontFamily === item.value && { boxShadow: true, shadow: theme.shadows[1] })}
                >
                  <Box sx={{ minWidth: 60, bgcolor: '#fff', p: 1, '&:hover': { bgcolor: '#f5f5f5' } }}>
